...
{
	if (libinput_device_config_scroll_get_methods(device) == 0)
		return LIBINPUT_SCROLL_METHOD_NONE;

	return device->config.scroll->get_default(device);
}
...
libinput_device_config_rotation_get_default(struct libinput_device *device)
{
	...
	return device->config.rotation->get_default(device);
}
					...
					enum libinput_accel_method method)
{
	...
	if (libinput_device_config_accel_get_method(device) == method)
		return LIBINPUT_CONFIG_STATUS_SUCCESS;

	return device->config.accel->set_method(device, method);
}

LIBINPUT_EXPORT enum libinput_accel_method
libinput_device_config_accel_get_method(struct libinput_device *device)
{
	if (!libinput_device_config_accel_is_available(device))
		return 0;

	return device->config.accel->get_method(device);
}
...
libinput_device_config_accel_get_default_method(struct libinput_device *device)
{
	...
	return device->config.accel->get_default_method(device);
}
